...
// 豆瓣图片代理函数 - 使用 wsrv.nl 图片代理服务解决 418/403 问题
function getDoubanImageProxy(originalUrl) {
    if (!originalUrl) return '';
    // wsrv.nl 是一个免费的图片代理/处理服务，可以绕过豆瓣的防盗链
    // 参数说明：url=原始图片地址, default=默认图片(可选)
    return `https://wsrv.nl/?url=${encodeURIComponent(originalUrl)}`;
}

// 备用图片代理列表
const IMAGE_PROXY_LIST = [
...
// 处理图片加载错误，尝试下一个代理
function handleDoubanImageError(img, originalUrl, proxyIndex = 0) {
    if (proxyIndex >= IMAGE_PROXY_LIST.length) {
        // 所有代理都失败，使用占位图
        img.src = 'data:image/svg+xml,' + encodeURIComponent(`
            <svg xmlns="http://www.w3.org/2000/svg" width="150" height="225" viewBox="0 0 150 225">
                <rect fill="#1a1a1a" width="150" height="225"/>
                <text fill="#666" font-family="sans-serif" font-size="14" x="50%" y="50%" text-anchor="middle" dominant-baseline="middle">暂无图片</text>
            </svg>
        `);
    ...
    const nextProxy = IMAGE_PROXY_LIST[proxyIndex];
    img.src = nextProxy(originalUrl);
    img.onerror = function() {
        handleDoubanImageError(img, originalUrl, proxyIndex + 1);
    };
}
...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        data.subjects.forEach(item => {
            ...
            // 处理图片URL - 使用图片代理服务解决 418/403 问题
            const originalCoverUrl = item.cover;
            // 主要使用 wsrv.nl 代理
            const proxiedCoverUrl = getDoubanImageProxy(originalCoverUrl);
            ...
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${proxiedCoverUrl}" alt="${safeTitle}" 
                        ...
                        onerror="handleDoubanImageError(this, this.dataset.originalUrl, 1)"
                        loading="lazy">
