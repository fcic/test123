...
// TMDB API 配置 - 用于获取电影/电视剧海报图片
// TMDB 是免费的电影数据库，提供高质量海报图片
// 用户可以在 localStorage 中设置 'tmdbApiKey' 来使用自己的 API key
const TMDB_CONFIG = {
    apiKey: '23894671180f2b4fcd581ee4c509b15a',// 'c1b0e735a17ad4d1d8ee5c4a2baa4a0f', // 公共测试key，建议替换为自己的
    imageBase: 'https://image.tmdb.org/t/p/w500',
    apiBase: 'https://api.themoviedb.org/3'
};

// 获取 TMDB API Key（优先使用用户配置的）
function getTMDBApiKey() {
    return localStorage.getItem('tmdbApiKey') || TMDB_CONFIG.apiKey;
}

// TMDB 搜索结果缓存，避免重复请求
const tmdbPosterCache = new Map();

// 从 TMDB 搜索电影/电视剧并获取海报URL
async function searchTMDBPoster(title, type = 'movie') {
    // 检查缓存
    ...
    // TMDB 的搜索类型：movie 或 tv
    const searchType = type === 'tv' ? 'tv' : 'movie';
    const url = `${TMDB_CONFIG.apiBase}/search/${searchType}?api_key=${apiKey}&query=${encodeURIComponent(title)}&language=zh-CN`;
    ...
    try {
        ...
        if (data.results && data.results.length > 0) {
            ...
            if (bestMatch.poster_path) {
                const posterUrl = TMDB_CONFIG.imageBase + bestMatch.poster_path;
...
function generatePlaceholderSVG(title) {
    ...
    return 'data:image/svg+xml,' + encodeURIComponent(`
        <svg xmlns="http://www.w3.org/2000/svg" width="150" height="225" viewBox="0 0 150 225">
            ...
            <rect fill="url(#grad)" width="150" height="225"/>
            <text fill="#e91e63" font-family="sans-serif" font-size="36" x="50%" y="40%" text-anchor="middle" dominant-baseline="middle">🎬</text>
            <text fill="#999" font-family="sans-serif" font-size="12" x="50%" y="60%" text-anchor="middle" dominant-baseline="middle">${displayTitle}</text>
        </svg>
    `);
}

// 处理图片加载错误 - 尝试从 TMDB 获取海报
async function handleDoubanImageError(img, title, type) {
    // 先显示加载中的占位图
    img.src = generatePlaceholderSVG(title);
    
    // 尝试从 TMDB 获取海报
    const tmdbPoster = await searchTMDBPoster(title, type);
    if (tmdbPoster) {
        img.src = tmdbPoster;
...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        // 当前是电影还是电视剧
        const contentType = doubanMovieTvCurrentSwitch;
        ...
        data.subjects.forEach(item => {
            ...
            // 使用 TMDB 图片作为主要来源
            // 先显示占位图，然后异步加载 TMDB 图片
            const placeholderUrl = generatePlaceholderSVG(item.title);
            
            // 为不同设备优化卡片布局
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${placeholderUrl}" alt="${safeTitle}" 
                        data-title="${safeTitle}"
                        data-type="${contentType}"
                        class="douban-poster w-full h-full object-cover transition-transform duration-500 hover:scale-110"
                        loading="lazy">
    ...
    // 异步加载所有海报图片
    loadAllPosters(container);
}

// 批量加载海报图片
async function loadAllPosters(container) {
    const posterImages = container.querySelectorAll('.douban-poster');
    
    // 使用 Promise.allSettled 并行加载，但限制并发数
    const batchSize = 4; // 每批加载4张图片
    const images = Array.from(posterImages);
    
    for (let i = 0; i < images.length; i += batchSize) {
        const batch = images.slice(i, i + batchSize);
        await Promise.allSettled(batch.map(img => loadSinglePoster(img)));
    }
}

// 加载单张海报
async function loadSinglePoster(img) {
    ...
    try {
        const posterUrl = await searchTMDBPoster(title, type);
        if (posterUrl) {
            // 预加载图片
            ...
            tempImg.onerror = function() {
                // TMDB 图片加载失败，保持占位图
                console.log(`Failed to load TMDB poster for: ${title}`);
            };
